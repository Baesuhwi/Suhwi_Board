$(document).ready(function(){
	...
	$("#btn-reply").click(function(){
		...
		$.ajax({
			...
			data : query2,
			success : function(data){
				var replyDiv = '<div class="reply_cont">' +
				"작성자: " + '<span id="id">' + query.id + "</span>" +
				" | 댓글: " + '<span id="reply_content">' + query2.reply_content + "</span>" +
				" | 작성 날짜: " + '<span id="reply_date">' + new Date().format("yyyy-MM-dd") + "</span>" +
				'</div>';
				console.log(id);
				console.log(reply_content);
				console.log(replyDiv);
				$(".replyDiv").append(replyDiv);
				$("#reply_content").val("");
			}, error : function(data){
				alert("fail");
			}
